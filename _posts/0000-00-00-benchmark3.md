...
> Hit peak FLOP
>
> Open your `peakflop.cpp` again and generalize (`template`) the type used in the `peak` function.
> Then use `BENCHMARK(peak<float>)` and `BENCHMARK(peak<simd::vec<float>>)` to instantiate the template for different 
> types.
{:.block-task}
...
void bench_ranges_count(benchmark::State &state) {
  for (auto _ : state) {
    for (auto s : test_strings) {
      benchmark::DoNotOptimize(ranges::count(s, ' '));
    }
  }
}
