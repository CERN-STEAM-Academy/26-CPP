...
As your codebase grows beyond a single file, keeping everything in one translation unit becomes unwieldy.
In real-world software, the core algorithms often live in libraries that multiple executables link against.
This section teaches you to extract the histogramming logic into a shared library.
...
>
> For this exercise either works.
> In general, shared libraries are more common because they can reduce memory usage and can be updated without 
> rebuilding every executable.
{:.block-task}
...
> {:.block-warning}
>
> You should arrive at a point where you have a function (in `histogram.h`) that determines (and returns) the index in the histogram.
